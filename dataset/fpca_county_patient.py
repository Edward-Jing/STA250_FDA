...
for i in selected:
    ...
    plt.plot(range(T1, T_total), death_target_test[i], label='True Patients Count (Last 40%)', linestyle='--')
    plt.plot(range(T1, T_total), predictions_test[i], label='Predicted Patients Count', marker='o', linestyle='--')
    ...
    # 添加代码：保存图片到fig文件夹下
    # -------------------------------
    save_path = f"fig/patients/county/fpca/patients_county_{county_id}_fpca.png"  # 定义保存路径
