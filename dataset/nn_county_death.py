...
# Merge the dataframes on 'county_id'
...
# 对于 workplace 数据，同样取前 T1 个时间作为输入特征
...
# -------------------------------
# 模型预测：使用预测分支获得后40%的预测结果
# 英文注释: Evaluate the model and obtain forecast results for the last 40% time steps
# -------------------------------
model.eval()
with torch.no_grad():
    X_tensor = torch.from_numpy(X).to(device)
    # 英文注释: Use the forecasting branch to obtain predictions for the last 40% time steps
...
for county in target_counties:
    # 在merged_df中查找county_id对应的索引（假设每个county_id唯一）
    county_indices = merged_df.index[merged_df['county_id'] == county].tolist()
    if len(county_indices) == 0:
        print(f"未找到 county {county} 的数据。")
        continue
    i = county_indices[0]  # 获取第一个匹配项的索引

    plt.figure(figsize=(10, 6))
    # 英文注释: Plot the observed death counts for the entire time series.
    plt.plot(range(T_total), death_ts[i], label='Observed Death Count', marker='o')
    ...
    plt.plot(range(T1, T_total), predictions[i], label='Predicted Death Count', marker='o', linestyle='--')
    ...
    # 添加代码：保存图片到fig文件夹下
    # -------------------------------
    save_path = f"fig/death/county/nn/death_county_{county}_nn.png"  # 定义保存路径
