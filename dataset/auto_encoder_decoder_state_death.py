...
# -------------------------------
# Load CSV files
death_df = pd.read_csv('death_state.csv')
...
for i, idx in enumerate(test_indices):
    ...
    # 绘制全部时间步的观察值
    plt.plot(range(T_total), death_ts_test[i], label='Observed Death Count', marker='o')
    # 绘制后40%时间步的真实死亡数
    plt.plot(range(T1, T_total), death_ts_test[i, T1:], label='True Death Count (Last 40%)', linestyle='--')
    # 绘制后40%时间步的预测值
    plt.plot(range(T1, T_total), test_predictions[i], label='Predicted Death Count', marker='o', linestyle='--')
    ...
    # 添加代码：保存图片到fig文件夹下
    # -------------------------------
    save_path = f"fig/death/state/auto_decoder/death_county_'{merged_df['state_id'].iloc[idx]}'_auto_decoder.png"  # 定义保存路径
