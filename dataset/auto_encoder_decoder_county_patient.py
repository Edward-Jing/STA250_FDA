...
for county in target_counties:
    ...
    # 绘制全部时间步的观测死亡数据
    plt.plot(range(T_total), death_ts_target, label='Observed Patients Count', marker='o')
    # 绘制后40%时间步的真实死亡数据
    plt.plot(range(T1, T_total), death_ts_target[T1:], label='True Patients Count (Last 40%)', linestyle='--')
    # 绘制后40%时间步的预测死亡数据
    plt.plot(range(T1, T_total), forecast_prediction, label='Predicted Patients Count', marker='o', linestyle='--')
    ...
    # 添加代码：保存图片到fig文件夹下
    # -------------------------------
    save_path = f"fig/patients/county/auto_decoder/patients_county_{county}_auto_decoder.png"  # 定义保存路径
