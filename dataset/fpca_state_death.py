...
for j in range(num_test):
    ...
    plt.plot(range(T1, T_total), death_target_test[j], label='True Death Count (Last 40%)', linestyle='--')
    # 使用预测结果中对应位置的数据
    plt.plot(range(T1, T_total), predictions_test[j], label='Predicted Death Count', marker='o', linestyle='--')
    ...
    # 保存图片到指定文件夹下
    # -------------------------------
    save_dir = "fig/death/state/fpca"
    save_path = os.path.join(save_dir, f"death_state_{state_id}_fpca.png")  # 定义保存路径
